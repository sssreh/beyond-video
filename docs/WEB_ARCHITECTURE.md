...
A first increment, deliberately browse/watch only - triggering `bv-download`/`bv-generate`/`bv-export` from the browser is intentionally not part of this yet (see `WORKING_CONTEXT.md`):

- **Login** - username/password, session cookie. Two roles: `owner` (currently just Christer) and `viewer` (everyone else). Nothing today is actually gated to `owner` beyond what `require_owner` makes possible for later use - browsing/watching trips works the same for both roles right now.
...
src/blackvue/web/
    ...
    templates/      Server-rendered Jinja2 templates (base/login/trip_list/
                   trip_detail/forbidden) - no client-side app to build,
                   no JSON API.
```
