...
A `--place` lookup, if given, is resolved first, printing its confirmation line before anything else - it's a one-off setup step (and, on a cache miss, a real network round-trip to Nominatim), not part of the timed search itself. After that, every run prints a `bv-search: started HH:MM:SS` line and a `bv-search: finished HH:MM:SS (N.Ns)` line before exiting, on every exit path from that point on (including argument errors) - a search over a wide date range on a large archive can take tens of seconds with nothing else printed in between, so both when it ran and how long it took are visible without timing it yourself. With `--trace`, a `.` is also printed to stdout every 25 recordings searched, as a "still active" heartbeat during a long run (same idea as `bv-download(1)`'s own `--trace`, just counting recordings instead of bytes).
