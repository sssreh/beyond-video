...
| `--frames N` | For `--describe-scene`: how many frames get described (default: `16`). Without `--adaptive-sampling` this is the plain even-spaced frame count handed to qwen_vl_utils' own sampling. With `--adaptive-sampling` it's how many highlighted moments to pick across the recording (one bullet per highlight in the output). Same underlying knob either way. Raising this was tried once before (`32`, then `64`) via a since-reverted `total_pixels`-budgeted video-sampling scheme, and caused a real quality regression (qwen_vl_utils' `VIDEO_MAX_PIXELS` ceiling got divided across more frames, roughly halving per-frame resolution). That budgeting code is gone now, so raising this just asks for more frames at full per-frame resolution - a speed/cost trade-off, not a known-bad path, but still untested at higher values since the revert. |
