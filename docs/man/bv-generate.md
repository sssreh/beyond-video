...
| `--adaptive-max-frames N` | With `--adaptive-sampling`: how many highlighted moments to pick across the recording (default: `16`, one bullet per highlight in the output). A different knob from `--adaptive-context-frames` - this one changes how many distinct moments get *described*, `--adaptive-context-frames` adds extra real frames of visual context *around* each one without adding more described moments. Raising this was tried once before (`32`, then `64`) via a since-reverted `total_pixels`-budgeted video-sampling scheme, and caused a real quality regression (qwen_vl_utils' `VIDEO_MAX_PIXELS` ceiling got divided across more frames, roughly halving per-frame resolution). That budgeting code is gone now, so raising this just asks for more highlights at full per-frame resolution - a speed/cost trade-off, not a known-bad path, but still untested at higher values since the revert. |
