...
| `--stitch-map-circle` | Mask the `--stitch-map` panel into a true circle (equal width and height, diameter = the shorter of the panel's own width/height), with the area outside it - corners, and a letterboxed margin along whichever axis is longer - rendered solid black. Christer: "do you think a zoomed map would look better as in a circle," then, after the first version stretched to an oval on non-square panels: "with a circle i mean a circle as wide as high." Off by default; only has an effect once `--stitch-map` is set. |
