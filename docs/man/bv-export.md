...
bv-export [--target DIR] [--config-dir DIR] [--prefix PREFIX]
          ...
          [--max-gap MINUTES] [--movement] [--gps-split] [--no-duration] [--duration-heal-archive]
...
Trip detection is shared with `bv-ls --trips`: `--max-gap`/`--movement`/`--gps-split`/`--no-duration`/`--gap-tolerance` all mean exactly the same thing here. `--max-parking-duration` and `--duration-heal-archive` are `bv-export`-only for now - `bv-ls --trips`'s preview doesn't apply either, so a trip split `--max-parking-duration` causes, or a `.duration.txt` `--duration-heal-archive` would have written, won't show up there ahead of time. Only recordings with Front video count toward trip detection - a recording with GPS/g-sensor/thumbnail data but no Front video (common if its video was never downloaded) never starts, extends, or belongs to a trip on its own; it's simply not part of any trip's export.
...
| `--gps-split` | Force a split between two recordings whose GPS position implies an impossible jump, even for a gap well within `--max-gap` - e.g. a stock/downloaded clip mixed into the archive that happens to land near real footage but was shot somewhere else entirely. **Off by default** - a real per-pair GPS probe (an EXIF read and/or an `ffprobe` subprocess) on every consecutive pair of recordings, see `bv-ls(1)`. |
...
A still photo (`.jpg`, `.jpeg`, `.png`, `.heic`, `.gpr`, plus a static/single-frame `.gif` - GoPro and folder archives only, see `CAMERA_ADAPTERS.md`) scanned into the archive is treated as a recording with only Front video and no Rear/Audio - Christer's own framing: "a picture is also a video, but 1 frame only." An animated `.gif` is treated as an ordinary silent video instead (it already has its own real per-frame timing), not a photo - see `CAMERA_ADAPTERS.md`'s "GIF classification and EXIF metadata" section for how the two are told apart. No flag is needed to include a photo; it's already part of whatever trip it falls into, same as any other recording.

| Option | Description |
|---|---|
| `--photo-duration SECONDS` | How long each photo plays for once it's part of an exported trip. Must be greater than 0. Default: 5. |

A photo's own image is scaled and letterboxed (not cropped, so nothing the photo's own framing chose to keep is cut off) to match the trip's real video dimensions and frame rate before being spliced into `front.mp4` - or, if the trip has no real video at all, sized against the photo's own pixel dimensions instead. A photo carrying an EXIF Orientation tag (a portrait phone photo, most commonly) is corrected before rendering, since ffmpeg does not auto-rotate on its own. A static `.gif` has its one real frame extracted first, before the same rendering step. A photo's EXIF DateTimeOriginal, if present, is preferred over file mtime when resolving its recording timestamp, and its EXIF GPS coordinates, if present, are shown on its archive-browser "Show location" page even though photos never generate a `.gps` sidecar. A photo that fails to render (a corrupt or unreadable image file) is left out of `front.mp4` entirely, with a warning, rather than failing the whole trip. Photos never contribute to `audio.aac`, `map.mp4`, or `--stitch-gsensor`/`--gsensor-video` overlays, since a still photo carries no audio or g-sensor data of its own.
...
| `--map-zoom [METERS]` | Render `map_zoom_METERSm.mp4`: a scrolling "follow camera" view, real-world half-width `METERS` (default 60 if given with no value - Christer, after trying it in practice: "Zoom is much better, now, 60 m or maybe even 30 should be default"). `METERS` also accepts a named preset instead of a number - `closeup` (30m), `medium` (60m), or `overview` (120m) - for expressing framing intent instead of guessing a number (there's no single radius that reads right for every trip - an intersection replay wants tight, a scenic drive wants wide). The position marker also scales bigger/smaller than normal the tighter/wider the resulting radius is relative to the plain 60m default. Independent of `--map` - works with or without it. |
