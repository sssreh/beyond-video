...
| Option | Description |
|---|---|
| `--gsensor-video` | Render `gsensor.mp4`: a dot moving on a gauge tracking lateral (left/right, the raw X field) vs. longitudinal (forward/back - acceleration/braking, the raw Y field) g-forces, with a fading trail, on chroma-key green - meant for compositing later, or via `--stitch-gsensor`. This X/Y axis mapping is Christer's own explicit call, not what a real recording with known turns and a braking event actually showed (that recording pointed to Y=lateral, Z=longitudinal instead); see `gsensor_render.py`'s module docstring for the full story and the follow-up recording planned to settle it either way. |
