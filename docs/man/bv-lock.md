...
A lock is scoped to two things at once: an exact time range (the same `--from`/`--until`/`--timestamp` selection every other `bv-*` command uses - see `docs/CLI.md`) and a set of asset names (`extract-audio`, `get-duration`, `transcribe`, `translate`, `srt`, `describe-scene`, `diarize` - the same vocabulary `bv-generate`'s own action flags map to, plus `all` as a convenience alias for every one of them). A range locked for `get-duration` only doesn't block a later `bv-generate --transcribe` run over the same range - only a flag combination that's a subset of what's already locked gets skipped. Locking is deliberately exact-range matching, not range-merging: a sub-range wholly inside an already-locked range (say, a single day inside a locked year) is treated as covered, but two overlapping-and-different ranges are never merged into one - lock (or unlock) the exact same selection you generated with, typically a whole year at a time. Locking `all` for one range and, say, `get-duration` for a different, overlapping range still leaves two separate manifest entries, neither of which alone covers a query spanning both.
...
| `--lock-assets ASSET[,ASSET...]` | Mark these asset types as done for the selected range. Comma-separated, from: `extract-audio`, `get-duration`, `transcribe`, `translate`, `srt`, `describe-scene`, `diarize` - or `all` for every one of them. |
| `--unlock-assets ASSET[,ASSET...]` | Remove these asset types from the lock for the selected range (also accepts `all`). A name that was never locked for that exact range is silently ignored, not an error. |
