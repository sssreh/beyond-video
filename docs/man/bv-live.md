...
- **G-sensor** (bottom, full width) - a scrolling strip of the last `--gsensor-window` seconds of live g-sensor readings. May be below the fold on a shorter window - scroll down to see it. Empty for the first few seconds after startup while it calibrates a zero-offset baseline from the camera's own readings (the sensor isn't necessarily centered on raw zero at rest); once calibrated, the strip's own vertical scale only ever grows to fit the biggest deviation seen so far in the session, never shrinks back down as an old peak scrolls out of view.
