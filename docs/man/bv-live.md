...
- **Camera** (top right) - the camera's own live MJPEG feed, proxied unchanged - not resized, cropped, or re-encoded, so its quality is exactly whatever the camera's own live-view stream provides. A Front/Rear/Interior button switches which direction is shown; only the direction currently on screen is actually being streamed from the camera. Not every camera has a real interior channel - some firmware answers the Interior button with a live-looking stream anyway (aliasing one of its real feeds) rather than a clean error, so its presence here isn't proof of a genuine third camera.
