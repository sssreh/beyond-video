...
An underground garage has no GPS signal, so a trip leaving home doesn't always start with a fix at home itself - the first fix the camera can resolve may land wherever GPS first locks on, which can sit outside `home_radius_meters` of the geocoded home point and so silently fail every home-adjacent match (Christer: "Usually when we drive north from garage, the GPS finds us at Skansbrogatan"). `driver_profiles.json`'s `home.extra_points` field (a list of `{"name", "query", "radius_meters"}` objects, hand-added the same way `default_from` is - never via a flag, and never seeded by this repo's own default profile) names one or more additional points that also count as "home" for every home-adjacent check this command and `bv-web`'s `/drivers` page do, each with its own independent radius rather than widening `home_radius_meters` itself (a larger primary radius was tried before and rejected - see `home_radius_meters`'s own tuning history in `trip/driver_detect.py` - because it started capturing unrelated nearby destinations as false "at home" matches).
