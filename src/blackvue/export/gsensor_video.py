...
ffmpeg. The gauge's horizontal axis is the raw X field (lateral -
left/right) and its vertical axis is the raw Y field (longitudinal -
forward/back, i.e. acceleration/braking) - Christer's own explicit
call, not what a real test recording actually showed; see
gsensor_render.py's module docstring for the full story and the
follow-up recording planned to settle it either way. See
gsensor_render.py for why the background is green rather than
transparent - h264/mp4 has no alpha channel, so a chroma-key
