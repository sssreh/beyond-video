...
def _sample(offset_ms, lateral, longitudinal, z=900):
    ...
    # (forward/back) - Christer's own explicit call, not what the real
    # test recording actually showed (see gsensor_render.py's module
    # docstring for the full story). Raw Z isn't used by either
...
def test_baseline_and_scale_for_samples_ignore_the_raw_z_field():
    # Regression guard for Christer's own X/Y mapping override (see
    # gsensor_render.py's module docstring) - raw Z isn't used by
    # either function, only X (lateral) and Y (longitudinal) are.
    # Wildly varying Z here should have zero effect on either result.
    samples = (
