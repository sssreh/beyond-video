...
def test_start_bv_scribe_defaults_reach_parsed_args(monkeypatch):
    ...
    assert args.fps == 1.0
    # 2026-08-19: bv-scribe's own --max-frames default briefly moved
    # 16->64->32 (see SceneOptions' own docstring - 64 measurably hurt
    # description quality the same day) before Christer asked to go
    # back to 16 outright - this test just confirms bv-web doesn't
    # override it, so it tracks that default.
